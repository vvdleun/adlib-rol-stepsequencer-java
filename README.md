...
*(Note that the step-sequencer itself is powered by text files as input and does not have a user-interface. The screenshot displayed above only shows a program that has a song open that was generated by this step-sequencer)*
...
* There can only be one track in a song right now (the stepsequencer is mono-trimbral). I expect to change this very soon.
